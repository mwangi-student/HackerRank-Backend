...
class AssessmentInvite(db.Model):
    ...
    id = db.Column(db.BigInteger, primary_key = True, autoincrement = True)
    assessment_id = db.Column(db.BigInteger, db.ForeignKey('assessment.id'), nullable= False)
    student_id = db.Column(db.BigInteger, db.ForeignKey('student.id'), nullable= False)
    invited_by = db.Column(db.BigInteger, db.ForeignKey('tm.id'), nullable= False)
    status = db.Column(db.String(255), nullable = False)

    assessment = db.relationship('Assessment', backref = db.backref('assessmentinvite', lazy = True))
    student = db.relationship('Students', backref= db.backref('assessmentinvite', lazy = True))
...
class Feedback(db.Model):
    ...
    student = db.relationship('Students', backref= db.backref('feedback', lazy = True))
...
class Submission(db.Model):
    ...
    student = db.relationship('Students', backref= db.backref('submission', lazy = True))
...
class Discussion(db.Model):
    ...
    student = db.relationship('Students', backref= db.backref('discussions', lazy = True))
...
class Leaderboard(db.Model):
    ...
    student = db.relationship('Students', backref= db.backref('leaderboard', lazy = True))
